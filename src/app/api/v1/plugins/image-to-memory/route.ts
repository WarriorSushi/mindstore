...
async function ensurePluginInstalled() {
  ...
  if ((existing as any[]).length === 0) {
    await db.execute(sql`INSERT INTO plugins (slug, name, description, type, status, icon, category) VALUES (${PLUGIN_SLUG}, 'Image-to-Memory', 'Upload images → AI describes → save as searchable memory.', 'extension', 'active', 'Image', 'ai')`);
  }
}

async function ensureImageTable() {
  await db.execute(sql`CREATE TABLE IF NOT EXISTS image_analyses (id UUID PRIMARY KEY DEFAULT gen_random_uuid(), user_id UUID NOT NULL, title TEXT, description TEXT, image_data TEXT, image_size INTEGER, image_format TEXT DEFAULT 'png', image_width INTEGER, image_height INTEGER, tags TEXT[] DEFAULT '{}', context_type TEXT DEFAULT 'general', provider TEXT, model TEXT, word_count INTEGER, saved_as_memory BOOLEAN DEFAULT false, memory_id UUID, custom_prompt TEXT, metadata JSONB DEFAULT '{}', created_at TIMESTAMPTZ DEFAULT NOW())`);
}

// ─── Vision Config ───────────────────────────────────────────

interface VisionConfig { type: 'openai' | 'gemini' | 'ollama' | 'openrouter' | 'custom'; key: string; model: string; url?: string; }

async function getVisionConfig(): Promise<VisionConfig | null> {
  const settings = await db.execute(sql`SELECT key, value FROM settings WHERE key IN ('openai_api_key','gemini_api_key','ollama_url','openrouter_api_key','custom_api_key','custom_api_url','chat_provider')`);
  const c: Record<string, string> = {};
  for (const row of settings as any[]) c[row.key] = row.value;
  ...
  if (p === 'openrouter' && orr) return { type: 'openrouter', key: orr, model: 'google/gemini-2.0-flash-001', url: 'https://openrouter.ai/api/v1/chat/completions' };
  ...
  if (orr) return { type: 'openrouter', key: orr, model: 'google/gemini-2.0-flash-001', url: 'https://openrouter.ai/api/v1/chat/completions' };
...
async function analyzeImage(config: VisionConfig, base64: string, mimeType: string, prompt: string) {
  if (config.type === 'gemini') {
    const res = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${config.model}:generateContent?key=${config.key}`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(buildGeminiVisionRequest(base64, mimeType, prompt)) });
    if (!res.ok) throw new Error((await res.json().catch(() => ({}))).error?.message || `Gemini Vision failed (${res.status})`);
    return extractTagsFromResponse((await res.json()).candidates?.[0]?.content?.parts?.[0]?.text || '');
  }
  if (config.type === 'ollama') {
    const url = (config.url || 'http://localhost:11434').replace(/\/$/, '');
    const res = await fetch(`${url}/api/generate`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(buildOllamaVisionRequest(config.model, base64, prompt)) });
    if (!res.ok) throw new Error(`Ollama vision failed (${res.status})`);
    return extractTagsFromResponse((await res.json()).response || '');
  }
  ...
  if (config.type === 'openrouter') { headers['HTTP-Referer'] = 'https://mindstore.app'; headers['X-Title'] = 'MindStore'; }
  ...
  if (!res.ok) throw new Error((await res.json().catch(() => ({}))).error?.message || `Vision API failed (${res.status})`);
  return extractTagsFromResponse((await res.json()).choices?.[0]?.message?.content || '');
}
...
export async function GET(req: NextRequest) {
  try {
    ...
    await ensurePluginInstalled(); await ensureImageTable();
    const action = req.nextUrl.searchParams.get('action') || 'images';

    if (action === 'images') {
      const limit = parseInt(req.nextUrl.searchParams.get('limit') || '50');
      const offset = parseInt(req.nextUrl.searchParams.get('offset') || '0');
      const images = await db.execute(sql`SELECT id, title, description, image_format, image_size, image_width, image_height, tags, context_type, provider, model, word_count, saved_as_memory, memory_id, created_at FROM image_analyses WHERE user_id = ${userId}::uuid ORDER BY created_at DESC LIMIT ${limit} OFFSET ${offset}`);
      const countResult = await db.execute(sql`SELECT COUNT(*) as total FROM image_analyses WHERE user_id = ${userId}::uuid`);
      return NextResponse.json({ images, total: parseInt((countResult as any[])[0]?.total || '0') });
    }

    if (action === 'stats') {
      const stats = await db.execute(sql`SELECT COUNT(*) as total_images, COUNT(*) FILTER (WHERE saved_as_memory = true) as saved_count, COALESCE(SUM(word_count), 0) as total_words, COALESCE(SUM(image_size), 0) as total_size, COALESCE(AVG(word_count), 0) as avg_words FROM image_analyses WHERE user_id = ${userId}::uuid`);
      const r = (stats as any[])[0] || {};
      return NextResponse.json({ totalImages: parseInt(r.total_images || '0'), savedCount: parseInt(r.saved_count || '0'), totalWords: parseInt(r.total_words || '0'), totalSize: parseInt(r.total_size || '0'), avgWords: Math.round(parseFloat(r.avg_words || '0')) });
    }
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (contentType.includes('multipart/form-data')) {
      ...
      await db.execute(sql`INSERT INTO image_analyses (id, user_id, title, description, image_data, image_size, image_format, tags, context_type, provider, model, word_count, custom_prompt) VALUES (${id}::uuid, ${userId}::uuid, ${title}, ${result.description}, ${thumbnailData}, ${file.size}, ${file.type.split('/')[1] || 'png'}, ${tagsArray}::text[], ${contextType}, ${config.type}, ${config.model}, ${wordCount}, ${customPrompt})`);
    ...
    if (action === 'save') {
      ...
      const content = formatImageMemoryContent(memoryTitle, image.description, image.tags || [], (image.context_type || 'general') as ContextType);

      let embedding: number[] | null = null;
      try { const e = await generateEmbeddings([content]); if (e?.length) embedding = e[0]; } catch {}
    ...
    if (action === 'reanalyze') {
      ...
      const contextType = (context || image.context_type || 'general') as ContextType;
